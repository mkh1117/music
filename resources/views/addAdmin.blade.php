    ...
    <div class="content">
       ...
       <div dir="rtl" class="col-lg-8 offset-lg-2" style="margin-top:2%;margin-top:2%;padding: 2%;backdrop-filter:blur(5px);border:2px solid rgba(255,255,255,.2); border-radius:20px; ">
        ...
        <div class="input-group" style="margin-top: 10px;">
            <form id="myForm" action="{{ route('addAdmin') }}" method="post" style="width:100%;">
                @csrf
                <div class="input-group col-12" style="margin-top: 10px;margin-bottom: 10px;">
                    <input type="text" placeholder="نام" name="name" id="typeText" class="form-control text-white back large-input" style="margin-left:10px; ">
                    @error('name')
                    <div style="color: red;" >{{ $message }}</div>
                    @enderror
                    <input type="text" placeholder="نام کاربری" name="username" id="typeText" class="form-control text-white back large-input">
                    @error('username')
                    <div style="color: red;" >{{ $message }}</div>
                    @enderror
                </div>
                ...
                @error('password')
                    ...
                    @enderror
                <input type="password" placeholder="تکرار رمز" name="passwordcheck" id="typeText" class="form-control text-white back large-input" style="margin-top: 1%">
                    @error('passwordcheck')
                    <div  class="large-input" style="color: red; text-align: right;" dir="rtl">{{ $message }}</div>
                    @enderror
