...
@section('navbar')
    ...
    <div class="content">
       ...
       <div dir="rtl" class="col-lg-8 offset-lg-2" style="margin-top:2%;margin-top:2%;padding: 2%;backdrop-filter:blur(5px);border:2px solid rgba(255,255,255,.2); border-radius:20px; ">
        <p class="h3 white" style="text-align: center;">تغییر نام کاربری</p>
        <div class="input-group" style="margin-top: 10px;">
            <form id="myForm" action="{{ route('changeusername') }}" method="post" style="width:100%;">
                @csrf
                <input type="text" name="username" id="typeText" value="{{ $username }}" class="form-control text-white back ">
                @error('username')
                <div style="color: red;text-align: right" >{{ $message }}</div>
                @enderror
                <input type="password" placeholder="رمز خود را وارد کنید" name="password" id="typeText" class="form-control text-white back " style="margin-top:1%;">
                    @error('password')
                    <div style="color: red;text-align: right" >{{ $message }}</div>
                    @enderror
