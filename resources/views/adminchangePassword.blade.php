...
@section('navbar')
    ...
    <div class="content">
       ...
       <div dir="rtl" class="col-lg-8 offset-lg-2" style="margin-top:2%;margin-top:2%;padding: 2%;backdrop-filter:blur(5px);border:2px solid rgba(255,255,255,.2); border-radius:20px; ">
        <p class="h3 white" style="text-align: center;">تغییر رمز عبور</p>
        <div class="input-group" style="margin-top: 10px;">
            <form id="myForm" action="{{ route('changePassword') }}" method="post" style="width:100%;">
                @csrf
                <input type="password" placeholder="رمز قبلی را وارد کنید" name="lastpassword" id="typeText" class="form-control text-white back ">
                @error('lastpassword')
                <div style="color: red;text-align: right" >رمز وارد شده نادرست است</div>
                @enderror
                <input type="password" placeholder="رمز جدید را وارد کنید" name="newpassword" id="typeText" class="form-control text-white back" style="margin-top: 1%">
                    @error('newpassword')
