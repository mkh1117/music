...
<div class=" bg-dark"  id="column" >
    <div class="card col-2 " style="margin-top:1%; " id="c1">
      ...
      <div class="row " id="wrapper" style="margin-top:1%;">
        @error('msg')
                <div class="alert alert-success" style="text-align: center;" dir="rtl"><h4>{{ $message }}</h4></div>
            @enderror
        <div class="card col-lg-12 offset-lg-0 col-md-12 offset-md-0 col-sm-12 offset-sm-0 col-12 black p-5 d-flex flex-row shadow " style="justify-content: center">
            <div class="d-flex align-items-center justify-content-between" style="flex-direction: column;width: 35%" dir="rtl">
                <h1 class="h3" style="text-align: center; color:#f6f7f7;">{{ $h1 }}</h1>
                <div class="contact-info">
                    <div><strong>ایمیل:</strong> {{ $gmail }}</div>
                    <div><strong>شماره تماس:</strong> {{ $phoneNumber }}</div>
                </div>
                <div class="contact-form" style="width: 100%">
                    <form id="myForm" action="{{ route('sendMessage') }}" method="POST" >
                        @csrf
                        <input type="text" name="name"  placeholder="نام و نام خانوادگی" required>
                        <input type="email" name="email" placeholder="ایمیل" required>
                        <textarea name="message" rows="5" maxlength="600" placeholder="پیام شما" required></textarea>
                        <button type="submit">ارسال پیام</button>
                    </form>
                </div>
                <div class="social-icons">
                    <p style="color:#f6f7f7">ما را در شبکه‌های اجتماعی دنبال کنید:</p>
