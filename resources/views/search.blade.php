...
<body >
    ...
    <nav class="navbar black col-12">
        ...
        <a class="navbar-brand  text-green sad" href="/sad" >آهنگ غمگین</a>
          <form class="form-inline my-2 my-lg-0" action="{{ Route('search') }}" method="post" style="display: flex">
            @csrf
...
<div class="container bg-dark"  id="column">
    <div class=" col-2 " style="margin-top:5%; " id="c1">
      ...
      <div id="c2" class=" col-2" style="margin-top:5%; background:linear-gradient(90deg,rgb(84, 83, 84)40%,rgb(110, 110, 112)60%,rgb(167, 167, 171)100%);display:flex; ">
       @foreach ($artists as $artist)
       <i class="bi bi-music-note-beamed">
        <a  style="text-decoration: none ;float:right; color:#000000;" href="/{{ $artist->name }}"><p > {{ $artist->name }} </p></a>
        </i>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="black" class="bi bi-music-note-beamed" viewBox="0 0 16 16">
        ...
        <div class="row " id="wrapper" style="margin-top:5%;">
            <header style=" background:linear-gradient(90deg,rgb(12, 12, 12)10%,rgb(50, 50, 49)50%,rgb(84, 83, 84)100%);height:40px;">
                ...
                <a href="/" class="text-green" style="text-decoration: none ; " title="دانلود آهنگ جدید">{{ $h1 }}</a>
                      </i>
            ...
            <div class="justify-content-center offset-3 linkDow">
                <a href={{ asset('/music/'.$main->music) }}  class="list-group-item text-center  fs-7 width:auto fw-bold" ><p style="margin-top: 10px">دانلود اهنگ</p></a>
            </div>
            </br>

                <audio src="/music/{{ $main->music }}" controls type="audio/mpeg" class="mx-auto col-md-12 col-12" style="margin-top: 10px;">
                </div>
                <div style="margin-top:5%;"></div>
            @endforeach
            <div class=" d-flex align-items-center justify-content-center offset-xl-0" style="margin-top: 10px;">
                @if ($page!=1)
            <a class="btn green " href="?page=1" role="button" style="pading= 5px;">{{ 1 }}</a>
                @endif
                @for ($i=$page-2;$i<$page;$i++)
                @if ($i>1 and $i<$number_page )
                <a class="btn green" href="?page={{ $i }} " role="button" style="pading= 5px;">{{ $i }}</a>
                @endif
                @endfor
            <a class="btn " href="?page={{ $page }} " role="button" style="pading= 5px; background-color:grey;" disabled>{{ $page }}</a>
           <?php $n=$page+2 ?>
            @for ($i=$page+1;$i<=$n;$i++)
                @if ($i>1 and $i<$number_page )
                <a class="btn green" href="?page={{ $i }} " role="button" style="pading= 5px;">{{ $i }}</a>
                ...
                @endfor
                @if ($page!=$number_page)
            <a class="btn green " href="/search/{{ $search }}/?page={{ $number_page }} " role="button" style="pading= 5px;">{{ $number_page }}</a>
                @endif
                </div>
            </div>
        </div>
